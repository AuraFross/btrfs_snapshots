...
generate_cron_file() {
    ...
    # NOTE: Unraid's crond (Dillon's crond 4.5) does NOT support variable
    # assignments (SHELL=, PATH=) in /etc/cron.d/ files — they cause parse
    # failures that silently drop the entire file. Keep header comments only.
    local -a lines=(
